...
const implementWinningToggle = (
  ...
  for (const losingFunction of losingArgumentFunctions) {
    for (const x of j(losingFunction.body).find(j.Identifier)) {
  ...
  return (
    source
      ...
      .forEach((importDef) => {
        ...
        for (const importSpecifier of j(importDef).find(j.ImportSpecifier, {
